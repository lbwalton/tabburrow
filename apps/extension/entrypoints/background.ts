...
import { captureAllWindows, snapshotsEqual } from "../lib/sessions";
...
export default defineBackground(() => {
  ...
  chrome.runtime.onInstalled.addListener((details) => {
    ...
    void setMeta("sessionState", "running");
  });
...
async function handleBrowserStartup(): Promise<void> {
  const previousState = await getMeta("sessionState");
  if (previousState === "running") {
    await setMeta("crashDetected", "1");
  }
  await setMeta("sessionState", "running");
}
...
async function handleWindowRemoved(): Promise<void> {
  ...
  if (remaining.length === 0) {
    await setMeta("sessionState", "endedClean");
  }
}
